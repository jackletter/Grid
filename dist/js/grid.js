(function (window, $) {
  var Grid = (window.Grid = function (ele, conf) {
    ...
    this._initColWid = function () {
      ...
      for (var i = 0; i < this.colWidths.length; i++) {
        var _wid = 0;
        if (
          typeof this.colWidths[i] == "string" &&
          this.colWidths[i].indexOf("%") > 0
        ) {
    ...
    this._initHeader = function () {
      _this.target_header = $(
        '<div class="grid-header grid-header-scrollbar"></div>'
      ).appendTo(target);
      ...
      for (var i = 0; i < this.headArr.length; i++) {
        ...
        for (var j = 0; j < this.headArr[i].length; j++) {
          ...
          var div = $("<div>" + this.headArr[i][j].text + "</div>");
          //全选
          if (this.headArr[i][j].type == "chk") {
            div = $("<div class='grid-checkbox'></div>");
            div.click(evt => {
              var _div = $(evt.target);
              _div.toggleClass("grid-checked");
              if (_div.hasClass("grid-checked")) {
                _this.selectAllRow();
              } else {
                _this.clearSlectRow();
              }
          ...
          if (this.headArr[i][j].order) {
            ...
            var icon = $("<div class='sort-icon'></div>")
              .appendTo(div)
              .data("data", this.headArr[i][j]);
          ...
          if (this.sizeColumn && i == this.headArr.length - 1) {
            var size = $("<div class='size-handle' />")
              .appendTo(div)
              .mousedown(this._sizeDown);
          }
        }
      }
    };
    ...
    this._delColTypes.lab = function (paras) {
      ...
      if (tdconf.click) {
        ...
        a
          .html(tdtext)
          .attr("data-name", tdconf.name)
          .attr("data-value", item[tdconf.name])
          .attr("title", "titleTxt");
        a.click(
          (function (tdconf, tdtext, item) {
            return function () {
      ...
      if (canHide) {
        ...
        if (_type == "text") {
          input = $(
            '<input style="display:none" dataName="' + tdconf.name + '" />'
          ).appendTo(div);
          input.val(tdtext);
        } else if (_type == "select") {
          if (!tdconf.edit.options)
            throw new Error("当单元格编辑类型为下拉框时必须指定options参数!");
          ...
          for (var h = 0; h < tdconf.edit.options.length; h++) {
            str +=
              '<option value="' +
              tdconf.edit.options[h].value +
              '">' +
              tdconf.edit.options[h].text +
              "</option>";
          }
          str += "</select>";
      ...
      if (typeof tdconf.dblClick == "function") {
        //单元格双击
        td.dblclick(
          (function (tdconf, tdtext, item, tr) {
            return function (evt) {
              evt.stopPropagation();
              tdconf.dblClick.apply(this, [tdtext, item, tr, div]);
            };
          })(tdconf, tdtext, item, tr)
        );
      }
    };
    ...
    this._delColTypes.btnEdit = function (paras) {
      ...
      var a_edit = $(
        "<a href='javascript:void(0)' btnFlag='__edit'>编辑</a>"
      ).appendTo(div);
      div.append("&nbsp;&nbsp;");
      var a_delete = $(
        "<a href='javascript:void(0)' btnFlag='__delete'>删除</a>"
      ).appendTo(div);
      var a_update = $("<a href='javascript:void(0)' btnFlag='__update'>更新</a>")
        .hide()
        .appendTo(div);
      div.append("&nbsp;&nbsp;");
      var a_cancel = $("<a href='javascript:void(0)' btnFlag='__cancel'>取消</a>")
        .hide()
        .appendTo(div);
    ...
    this._addRows = function (rows) {
      ...
      var rowindex = 0;
      if (indexs.length > 0) {
        rowindex += window.parseInt(
          indexs.eq([indexs.length - 1]).attr("data-row-index")
        );
      } else if (this.isPage) {
    ...
    this._initBody = function (res) {
      ...
      if (res.data.length == 0) {
        //没有数据时插入提示行
        var tr = $(
          '<tr class="grid-no-data"><td colspan="' +
    ...
    this._edit = function (tr) {
      ...
      for (var i = 0; i < tds.length; i++) {
        ...
        td
          .find("input")
          .val(td.find("span[canHide=true],a[canHide=true]").text())
          .show();
        td
          .find("select")
          .val(td.find("span[canHide=true],a[canHide=true]").attr("data-value"))
          .show();
      }
    ...
    this._delete = function (tr) {
      var data = tr.data("rowdata");
      if (!_this.onDelete) {
        throw new Error("必须定义onDelete方法!");
      }
      _this.onDelete(data);
    }
    ...
    this._updateClick = function () {
      if (!_this.onUpdate) {
        throw new Error("必须定义onUpdate方法!");
      }
      var tr = $(this).parentsUntil("tbody", "tr");
      _this._update(tr);
    };
    this._cancel = function (tr) {
      var tds = tr.children("[data-edit=true]");
      for (var i = 0; i < tds.length; i++) {
        var td = tds.eq(i);
    ...
    this._initFooter = function (res) {
      if (this.isPage && this.pageSize > 0) {
        ...
        if (this.pageIndex == 1) {
          div.append(
            '<span class="page-first page-disabled" data-page="first"></span>'
          );
          div.append(
            '<span class="page-prev page-disabled" data-page="prev"></span>'
          );
        } else {
          $('<span class="page-first" title="首页" data-page="first"></span>')
            .appendTo(div)
            .click(this._onPageChange);
          $('<span class="page-prev" title="上一页" data-page="prev"></span>')
            .appendTo(div)
            .click(this._onPageChange);
        }
        div.append(
          '<span class="separator"></span><span style=" margin: 5px;">第</span>'
        );
        var input = $(
          '<input type="text" style="width:50px; height:25px; line-height:25px;" value="' +
          this.pageIndex +
          '" data-page="direct" />'
        )
          .on("change", this._onPageChange)
          .appendTo(div);
        div.append('<span style="margin:5px;">页,共' + pageCount + "页</span>");
        div.append('<span class="separator"></span>');
        if (this.pageIndex == pageCount || pageCount == 0) {
          div.append(
            '<span class="page-next page-disabled" data-page="next"></span>'
          );
          div.append(
            '<span class="page-last page-disabled" data-page="last"></span>'
          );
        } else {
          $('<span class="page-next" title="下一页" data-page="next"></span>')
            .appendTo(div)
            .click(this._onPageChange);
          $('<span class="page-last" title="尾页" data-page="last"></span>')
            .appendTo(div)
            .click(this._onPageChange);
        }
        var currentIndex = (this.pageIndex - 1) * this.pageSize + 1;
        div.append(
          '<span class="separator"></span><span style="margin: 5px;">当前显示第' +
          (currentIndex < 0 ? 0 : currentIndex) +
          "条到第" +
          (currentIndex + res.data.length - 1 < 0
            ? 0
    ...
    this._onOrder = function (evt) {
      ...
      if (box.length == 0) {
        ...
        $('<div class="grid-sort-item" data-type="asc" />')
          .appendTo(box)
          .html("正序")
          .click(_this._orderClick);
        $('<div class="grid-sort-item" data-type="desc" />')
          .appendTo(box)
          .html("倒序")
          .click(_this._orderClick);
        $('<div class="grid-sort-item" data-type="none" />')
          .appendTo(box)
          .html("取消")
          .click(_this._orderClick);
      ...
      if (srcType != targetType) {
        ...
        if (_this.isPage) {
          ...
          _this.onQuery(
            _this.pageIndex,
            _this.pageSize,
            _this._sortOrders(_this._collectOrders())
          );
        } else {
    ...
    this._collectOrders = function (arr, orders) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        if (Array.isArray(item)) {
          orders = _this._collectOrders(item, orders);
        }
        if (item.order) {
          orders.push({
    ...
    this._sizeDown = function (evt) {
      ...
      $(document.body)
        .bind("mousemove", _this._sizeMove)
        .bind("mouseup", _this._sizeUp);
    ...
    this._sizeMove = function (evt) {
      ...
      if (widt <= 10) return;
      var sw =
        _this.target_body.width() - _this.target_body.find(">table").width();
      if (dx < 0 && sw == 16) {
    ...
    this._sizeUp = function () {
      ...
      $(document.body)
        .unbind("mousemove", _this._sizeMove)
        .unbind("mouseup", _this._sizeUp);
      $(document.body).removeAttr("onselectstart");
    };
    this.setHeight = function (height) {
      _this.target_body.css("height", height);
    };
    //选中指定行
    this.selectRow = function (rows) {
      rows = $(rows);
      _this.clearSlectRow();
      var tbody = _this.target_body.find("tbody");
      rows.addClass("grid-select-row");
      rows.find(".grid-checkbox").addClass("grid-checked");
    }
    //选中所有行
    ...
    //清空选择
    this.clearSlectRow = function () {
      var tbody = _this.target_body.find("tbody");
      tbody.children().removeClass("grid-select-row");
      tbody.find(".grid-checkbox").removeClass("grid-checked");
    }
    //开启行编辑
    this.beginEditRow = function (tr) {
      /*//将其他在编辑的行都取消掉
      var tbody = _this.target_body.find("tbody");
      var edits = tbody.children("[in-edit]");
      edits.each((index, tr) => {
        _this._cancel($(tr));
        $(tr).removeAttr("in-edit");
      });
      tr.attr("in-edit", "true");
      _this._edit($(tr));   */
    }
  ...
  Grid.settings = {
    ...
    pageIndex: 1, //当前页索引
    onQuery: function () {
      throw new Error("必须自定义onQuery方法!");
    },
